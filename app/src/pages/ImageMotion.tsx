...
import { sameBlockReasonsForUI } from '../lib/motion-guard';
...
export const ImageMotion: React.FC = () => {
  ...
  // 仅播放当前 Region 的 spec 覆盖；为 null 时预览仍用全片 spec（顶部「预览」按钮）。
  const [regionPreviewSpec, setRegionPreviewSpec] = useState<AnimSpec | null>(null);
  ...
  // 清除动画（默认层）：保留 Region 框选，把该 Region 的 actions[] 清空，让用户能重选。
  // 与「删除此区域」（删整条 Region + 所有 Actions）和调整细节里逐条 ×（删单个 Action）保持区分。
  const clearItemActions = (itemId: string) => {
    setItems((prev) => prev.map((i) => i.id !== itemId ? i : { ...i, actions: [] }));
    clearFeeling(itemId);
  };
  ...
  // 预览此区域：用单 region 临时 spec 起一个 preview，不必每次都从 Region 1 看到这里。
  // 顶部「预览」仍然预览整片（不被覆盖）。
  const openRegionPreview = () => {
    if (!image || !sel || sel.actions.length === 0) return;
    const s = buildSpecA({
      imageSrc: image.dataUrl, imageW: image.w, imageH: image.h,
      aspect, items: [sel],
    });
    setRegionPreviewSpec(s);
    setShowPreview(true);
  };
  const closePreview = () => { setShowPreview(false); setRegionPreviewSpec(null); };
  ...
  // Motion Guard：哪些 Region 的「同时」无法可靠自动修复（禁用 + 一句人话原因）
  const sameBlocked = useMemo(() => {
    if (!image) return items.map(() => null);
    const dims = ASPECTS[aspect];
    return sameBlockReasonsForUI({
      items, imageW: image.w, imageH: image.h, canvasW: dims.w, canvasH: dims.h,
    });
  }, [image, items, aspect]);
        ...
        <div className="col" style={{ gap: 14 }}>
          {sel ? (
            <div className="card col" style={{ gap: 14 }}>
              ...
              {/* 清除动画（默认层）：保留框选，只把当前区域的 actions[] 清空。
                  与「删除此区域」（删整条 Region + 所有 Actions）语义不同。 */}
              {sel.actions.length > 0 && (
                <button className="ghost" style={{ alignSelf: 'flex-start', fontSize: 13 }}
                  title="保留框选，只清空当前区域的动作序列"
                  onClick={() => clearItemActions(sel.id)}>
                  清除动画
                </button>
              )}
              ...
              {sel.actions.length > 0 && (
                <button className="ghost" style={{ alignSelf: 'flex-start', fontSize: 13 }}
                  title="只播放当前区域，不必从头跑整段动画"
                  onClick={openRegionPreview}>
                  预览此区域
                </button>
              )}
              <button className="ghost" style={{ alignSelf: 'flex-start', fontSize: 13 }}
              ...
              {detailsFor === sel.id && (
                <div className="col" style={{ gap: 10, borderTop: '1px solid var(--stroke)', paddingTop: 12 }}>
                      ...
                      return (
                        <div key={a.id} className="card" style={{ padding: '8px 10px', gap: 6 }}>
                          <div className="row" style={{ gap: 6 }}>
                            ...
                            <button className="ghost" style={{ padding: '0 6px' }} onClick={() => moveActionInChain(sel.id, a.id, 1)}>↓</button>
                            <button className="ghost" style={{ padding: '0 8px', color: 'var(--danger)' }} title="删除此条 Action（不影响其他 Action）"
                              onClick={() => removeAction(sel.id, a.id)}>×</button>
                          </div>
                  ...
                  <button className="ghost" style={{ color: 'var(--danger)', alignSelf: 'flex-start' }} title="删除整条 Region（连同所有 Actions）"
                    onClick={() => removeItem(sel.id)}>删除此区域</button>
                </div>
              )}
            </div>
          ) : (
              ...
              {items.map((it, idx) => (
                <div key={it.id} className="anim-item" style={{ borderColor: selected === it.id ? 'var(--accent)' : undefined }}>
                  <div className="head">
                    ...
                    <button className="ghost" style={{ padding: '2px 6px' }} title="上移"
                      onClick={() => moveItem(it.id, -1)}>↑</button>
                    <button className="ghost" style={{ padding: '2px 6px' }} title="下移"
                      onClick={() => moveItem(it.id, 1)}>↓</button>
                    <button className="ghost" style={{ padding: '2px 8px', color: 'var(--danger)' }}
                      title="删除此区域（连同所有 Actions）"
                    ...
                    <div className="row" style={{ marginTop: 8 }}>
                      <span className="seg">
                        {(['same', 'after', 'later'] as const).map((r) => {
                          const blocked = r === 'same' ? sameBlocked[idx] : null;
                          return (
                            <button key={r}
                              className={it.relation === r ? 'active' : ''}
                              disabled={!!blocked}
                              title={blocked ?? undefined}
                              onClick={() => { if (!blocked) updateItem(it.id, { relation: r }); }}>
                              {RELATION_LABEL[r]}
                            </button>
                          );
                        })}
                      ...
                      {sameBlocked[idx] && (
                        <span className="muted" style={{ fontSize: 12, marginLeft: 6 }}>{sameBlocked[idx]}</span>
                      )}
                    </div>
                  )}
                </div>
