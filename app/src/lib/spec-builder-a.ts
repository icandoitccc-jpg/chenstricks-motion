...
import { actionMeta, guardPlan } from './motion-guard';
...
// 注：跨 Region 的镜头复位 / 延迟 /「同时」冲突判定全部由 motion-guard.ts 统一处理（单一入口）。
// 本文件只负责把 AItem[] 翻译成 SpecAction，不再针对某个 action 写补丁分支。
...
export function buildSpecA(opts: {
  ...
  // ---- Motion Guard：唯一入口 ----
  // 所有「会坏 / 会看不到 / camera 或 state 冲突」的处理都在这里产出，
  // 不要在 buildSpecA 里再写针对某个 action 的 if/else 补丁。
  const active = items.filter((i) => i.actions.length > 0);
  const canvasRegions = active.map((i) => toCanvas(i.region));
  const chainFrames = active.map((it) => {
    let n = 0;
    it.actions.forEach((a, i) => {
      n += actionTotalFrames(a, it);
      if (i < it.actions.length - 1) n += CHAIN_GAP_FRAMES;
    });
    return n;
  });
  // 链内最后一个「会改 camera」的动作的结束偏移；没有则为 -1
  const camEndOffset = active.map((it) => {
    let off = 0;
    let found = -1;
    it.actions.forEach((a, i) => {
      const dur = actionTotalFrames(a, it);
      if (actionMeta(a.action).mutatesCamera) found = off + dur;
      off += dur;
      if (i < it.actions.length - 1) off += CHAIN_GAP_FRAMES;
    });
    return found;
  });
  ...
  const plan = guardPlan({
    ...
    canvasW: cw,
    canvasH: ch,
    headPadding: HEAD_PADDING_FRAMES,
    laterGap: RELATION_LATER_GAP,
  });
  ...
  // Guard 自动插入的镜头复位（隐形补偿动作，用户不需要知道）
  for (const p of plan.pulls) {
    actions.push({
      action: 'cameraPull', target: 'camera',
      at: p.at, duration: p.duration,
      speed: 'normal', intensity: 'normal',
    });
  }
