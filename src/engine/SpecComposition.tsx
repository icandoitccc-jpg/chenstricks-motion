...
// 退出类动作在光栅区域上压一层暗罩（veil）。
// veil 是「临时视觉状态」：如果该元素后面还有其他 Action，必须在下一个 Action 开始前释放，
// 否则它会一直压着、污染后续所有 Action（state leak → 后续动作看不见）。
// 但如果退出动作本来就是序列的最后一步，则正常保持退出结果（用户就是要它退场）。
// 注意：不靠「把退出动作挪到最后」来解决 —— 那是擅自改动用户的 Action 顺序。
function exitVeil(elActions: SpecAction[], at: number, dur: number, frame: number, peak: number): number {
  let nextAt = Infinity;
  for (const o of elActions) {
    const oAt = o.at ?? 0;
    if (oAt > at && oAt < nextAt) nextAt = oAt;
  }
  if (nextAt === Infinity) return peak; // 终态：保持退出结果
  const relStart = at + dur;
  const relEnd = nextAt;
  const r = relEnd > relStart
    ? Math.min(1, Math.max(0, (frame - relStart) / (relEnd - relStart)))
    : (frame >= relStart ? 1 : 0);
  return peak * (1 - r);
}
...
function computeMotion(
  ...
  for (const a of actions) {
    ...
    // 光栅区域：副本像素与底图完全相同，纯 opacity/scale→1 在原位 = 看不见。
    // 用「动作本身的可视性」做信号，不再靠阴影/圆角/额外放大 —— 那些会让矩形选区
    // 变成一张悬浮卡（用户实测：白色矩形 + 阴影）。scaleEmphasis/pulse/popIn/slideIn
    // 各自的 transform 已经足以让边缘延伸；fadeIn 在 raster 上单独补一个轻微 content-edge
    // scale pop（见 fadeIn case），不动矩形 div 本身。
    const lift = (_mul: number) => { /* no-op：避免出现悬浮白卡 */ };
    ...
    switch (a.action) {
      case 'fadeIn': {
        ...
        // 光栅区域：opacity 在同像素副本上等于看不见。补一个轻微 content-edge pop
        // （让 logo 自己的边缘从 0.96 弹到 1），动作仍然明显但不暴露矩形边界。
        if (isRegion) m.scale = Math.min(m.scale, interpolate(easeOut(t), [0, 1], [0.96, 1]));
        break;
      }
      case 'popIn': {
      ...
      case 'fadeOut': {
        ...
        if (isRegion) m.veil = Math.max(m.veil, exitVeil(actions, at, dur, frame, 0.78 * pp));
      ...
      case 'slideOut': {
        ...
        const pp = easeOut(t);
        if (isRegion) {
          m.veil = Math.max(m.veil, exitVeil(actions, at, dur, frame, 0.78 * pp));
...
const ElementView: React.FC<{
  ...
  if (el.kind === 'image' || el.kind === 'imageRegion') {
    ...
    // 关键约束：副本只是定位/mask 信息，不应该成为最终视觉对象 —— 不能让它变成
    // 一张带圆角和阴影的悬浮卡（用户实测：白色矩形 + 阴影）。所以：无 borderRadius、
    // 无 boxShadow、不在矩形 div 上加额外 liftScale，让矩形的方形边界与底图像素保持
    // 严丝合缝，可见性完全交给「动作本身的 transform」（scaleEmphasis / pulse / popIn /
    // slideIn / fadeIn 的轻微 content-edge pop）。
    const r = el.region!;
    ...
    const regionStyle: React.CSSProperties = {
      ...
      width: el.w ?? r.w,
      height: el.h ?? r.h,
      transform: `translate(${m.translateX}px, ${m.translateY}px) scale(${m.scale})`,
    };
    return (
